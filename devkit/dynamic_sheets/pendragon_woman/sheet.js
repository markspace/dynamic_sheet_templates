/*
* This is the javascript specific to the pendragon_woman DST 
* by Mark Davidson
*
...
// Global Storage
pendragon_woman_context = {};
var $ = jQuery

var pendragon_woman_default_values = {
  ...
  "combat_battle" : "1",
  "combat_horsemanship" : "3",
  "combat_dagger" : "5",

};

function pendragon_woman_dataPreLoad(opts) {
    ...
    var tempData = {};
    for (var val in pendragon_woman_default_values)
    {
        tempData[val] = pendragon_woman_default_values[val];
    }
...
function pendragon_woman_dataPostLoad(opts) {

    var containerId = "#" + opts['containerId'];
    pendragon_woman_context = document.getElementById(opts['containerId']);
    opts['context'] = document.getElementById(opts['containerId']);

    //Convert interface elements
    pendragon_woman_convertAreas(opts);
    ...
    pendragon_woman_derivedStats();
}

function pendragon_woman_dataChange(opts) {

    pendragon_woman_derivedStats();

}

function pendragon_woman_dataPreSave(opts) {
    opts['context'] = document.getElementById(opts['containerId']);
    pendragon_woman_unconvertAreas(opts);
    var containerId = "#" + opts['containerId'];
}

function pendragon_woman_derivedStats() {
    var size = pendragon_woman_getInt(jQuery('.dsf_siz').html());
    var dexterity = pendragon_woman_getInt(jQuery('.dsf_dex').html());
    var strength = pendragon_woman_getInt(jQuery('.dsf_str').html());
    var constitution = pendragon_woman_getInt(jQuery('.dsf_con').html());
    var appearance = pendragon_woman_getInt(jQuery('.dsf_app').html());
...
function pendragon_woman_getInt(val) {
    var score = parseInt(val);
    if (isNaN(score)) score = 0;
    return score;
}

function pendragon_woman_area(oElement,opts){
    ...
    oElement.edit = function(){
        ...
        var aAreas = pendragon_woman_getElementsByClassName('area','span');
        ...
        this.innerHTML = '<textarea class=pendragon_woman_textarea "' + sClasses + '" style="width:' + iWidth + 'px;height:' + iHeight + 'px;">' + sText + '</textarea>';

        var focusRef = function(){$('.pendragon_woman_textarea').focus();};
        var submitRef = function(){$('.pendragon_woman_textarea').parent().submit();};
        var unfocusRef = function(){$('.pendragon_woman_textarea').blur(submitRef)};
...
function pendragon_woman_convertAreas(opts){
    ...
    for (var i = 0; i < aSpans.length; i++){
        if (aSpans[i].className.match(/area/)){
            ...
            taTemp = pendragon_woman_area(aSpans[i],opts);
            taTemp.activate();
...
function pendragon_woman_unconvertAreas(opts){
...
function pendragon_woman_getElementsByClassName(sClassName,sElementType){

    // Provide default element type
    if (!sElementType) sElementType = 'div';

    var aList = new Array();
    var aDivs = pendragon_woman_context.getElementsByTagName(sElementType);
